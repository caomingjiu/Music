  ...
  <div class="cc-donghua">
      <div class="cc-col-center">
        ...
        <div v-for="(item, index) in musics" :key="index">
         ...
         <div>
        <audio :src="musicUrl" @play="ready" @pause="pause" ref='audioExample' controls></audio>
         </div>
          <div class="cc-df">
          <button class="cc-btn btn-size" @click="dele()">上一页</button>
          <input type="number" v-model="page" class="input-size cc-mright cc-mleft" />
          <button class="cc-btn btn-size" @click="add()">下一页</button>
        </div>
    </div>
  </div>
</template>
